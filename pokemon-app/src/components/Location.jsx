...
function Location({ onOwnPokemonSelect, areaPokemonsUrl, userPokemonsURL }) {
    ...
    fetch(areaPokemonsUrl)
      ...
      .then((url) => {
        Promise.all([...userPokemonsURL.map((p) => fetch(p)), fetch(url)])
          ...
          .then((data) => {
            setUserPokemons(data.slice(0, data.length - 1));
            console.log(data);
  ...
  return (
    <>
      {encounterPokemon && userPokemons && (
        <div className="ownPokemons">
          {userPokemons.map((pokemon, index) => (
            <div
              key={index}
              data-url={userPokemonsURL[index]}
              onClick={handleCLick}
            >
              <div></div>
              <FightingPokemon className="own" pokemon={pokemon} />
            </div>
          ))}
        </div>
      )}
      <div>
        <FightingPokemon className="encountered" pokemon={encounterPokemon} />
      </div>
    </>
  );
}
